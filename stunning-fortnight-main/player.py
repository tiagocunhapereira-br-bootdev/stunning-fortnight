...
class Player(CircleShape):
    ...
    def strafe(self, dt, direction):
        right_vector = pygame.Vector2(1, 0).rotate(self.rotation)
        self.position += right_vector * PLAYER_SPEED * dt * direction
    ...
    def update(self, dt, asteroid_group, targets):
        ...
        if direction.length_squared() > 0:
            ...
            self.rotation = math.degrees(angle_rad) - 90
        # Movement forward/backward with W/S keys
        if keys[pygame.K_w]:
            self.move(dt)
        if keys[pygame.K_s]:
            self.move(-dt)
        # Movement side-to-side with A/D keys
        if keys[pygame.K_a]:
            self.strafe(dt, -1)
        if keys[pygame.K_d]:
            self.strafe(dt, 1)

        # Shooting and abilities
    ...
    def fire_railgun(self, targets):
        ...
        for target in targets:
            # 2. Vector from ship to target
            vec_to_asteroid = target.position - self.position
            ...
            if projection_length > 0:
                ...
                if dist < target.radius:
                    self.score += target.handle_hit()
                    hit_count += 1

        RailgunBeam(self, targets)
        return hit_count
